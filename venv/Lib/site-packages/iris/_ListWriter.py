...
class _ListWriter(object):
    ...
    def _set(self, data, retainEmptyString=False):
        if retainEmptyString and type(data) == str and len(data) == 0:
            ...
            self.offset = iris._DBList._DBList._set(self.buffer, self.offset, data, self._locale, self._is_unicode, self._compact_double)
        return
    ...
    def _set_list(self, data):
        if data is None:
            ...
            self.offset = iris._DBList._DBList._stuff_bytes(self.buffer, self.offset, data._get_buffer())

    def _set_undefined(self):
        self.__check_buffer_size(1)
        self.offset = iris._DBList._DBList._set_undefined(self.buffer, self.offset)

    def _set_null(self):
        self.__check_buffer_size(2)
        self.offset = iris._DBList._DBList._set_null(self.buffer, self.offset)
    ...
    def _set_parameter(self, param):
        param_switcher = {
            iris.dbapi._DBAPI.SQLType.LONGVARBINARY: self._set_stream,
            iris.dbapi._DBAPI.SQLType.LONGVARCHAR: self._set_stream,
            iris.dbapi._DBAPI.SQLType.DATE: None,
            iris.dbapi._DBAPI.SQLType.TIME: None,
            iris.dbapi._DBAPI.SQLType.TIMESTAMP: None,
            iris.dbapi._DBAPI.SQLType.TYPE_DATE: None,
            iris.dbapi._DBAPI.SQLType.TYPE_TIME: None,
            iris.dbapi._DBAPI.SQLType.TYPE_TIMESTAMP: None,
            iris.dbapi._DBAPI.SQLType.DATE_HOROLOG: self._set_date_h,
            iris.dbapi._DBAPI.SQLType.TIME_HOROLOG: self._set_time_h,
            iris.dbapi._DBAPI.SQLType.TIMESTAMP_POSIX: self._set_posix
        }
    ...
    def _set_saved_offset_type_as_pass_by_reference(self):
        iris._DBList._DBList._set_type_as_pass_by_reference(self.buffer, self.__saved_offset)
        return
